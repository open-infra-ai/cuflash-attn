...
| `BUILD_BENCHMARKS` | ON | 构建 Google Benchmark 基准测试（`cuflash_bench`） |
...
```bash
# 运行所有测试
./build/release/cuflash_tests

# 运行特定测试套件
./build/release/cuflash_tests --gtest_filter="ForwardTest*"

# 列出所有可用测试
./build/release/cuflash_tests --gtest_list_tests
```
...
**自定义库路径：**
```bash
CUFLASH_LIB=/path/to/libcuflash.so python tests/integration/test_pytorch_comparison.py
```
...
docker run --gpus all cuflash ./build/release/cuflash_tests
```
